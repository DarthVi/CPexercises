...
def solve_nqueens_ch_1_rc(n):
    ...
    solver.Add(solver.AllDifferent(columns))

    for i in range(n):
        for j in range(n):
            if i < j:
                solver.Add(abs(rows[i] - rows[j]) != abs(i - j))
                solver.Add(abs(columns[i] - columns[j]) != abs(i - j))

    # global constraint that does the channeling
    solver.Add(solver.InversePermutationConstraint(rows, columns))

    db = solver.Phase(columns + rows, solver.INT_VAR_DEFAULT, solver.INT_VALUE_DEFAULT)
